...
type mockStsTokenSource struct {
	mock.Mock
}
...
func (m *mockStsTokenSource) Token() (*oauth2.Token, error) {
	args := m.Called()
	if args.Get(0) == nil {
...
func TestStsTokenSource(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:          "Failed to read KSA token",
			ksaTokenPath:  "non_existent_path",
			mockSetup:     func(m *mockRoundTripper) {},
			expectedError: "failed to read KSA token from file",
		},
		{
			name:         "AIS request failure",
			ksaTokenPath: mockKsaTokenFile.Name(),
			mockSetup: func(m *mockRoundTripper) {
				m.On("RoundTrip", mock.Anything).Return((*http.Response)(nil), fmt.Errorf("network error"))
			},
			expectedError: "failed to exchange token: Post \"test_ais_url\": network error",
		},
		{
			...
			expectedError: "failed to parse AIS json response",
		},
		{
