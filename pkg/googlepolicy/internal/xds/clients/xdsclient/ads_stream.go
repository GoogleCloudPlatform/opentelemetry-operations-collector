...
import (
	...
	igrpclog "github.com/GoogleCloudPlatform/opentelemetry-operations-collector/pkg/googlepolicy/internal/xds/clients/internal/grpclog"
	"github.com/GoogleCloudPlatform/opentelemetry-operations-collector/pkg/googlepolicy/internal/xds/clients"
	"github.com/GoogleCloudPlatform/opentelemetry-operations-collector/pkg/googlepolicy/internal/xds/clients/internal/backoff"
	"github.com/GoogleCloudPlatform/opentelemetry-operations-collector/pkg/googlepolicy/internal/xds/clients/internal/pretty"
...
type adsStreamOpts struct {
	...
	backoff            func(int) time.Duration              // Backoff for retries, after stream failures.
	timeAfter          func(time.Duration) <-chan time.Time // Optional sleep function for backoff retries.
	nodeProto          *v3corepb.Node                       // Node proto to identify the gRPC application.
	watchExpiryTimeout time.Duration                        // Resource watch expiry timeout.
	logPrefix          string                               // Prefix to be used for log messages.
	logger             grpclog.DepthLoggerV2                // Optional custom logger.
}
...
func newADSStreamImpl(opts adsStreamOpts) *adsStreamImpl {
	...
	s.logger = prefixLoggerWithParent(opts.logger, opts.logPrefix+fmt.Sprintf("[ads-stream %p] ", s))
...
func (s *adsStreamImpl) runner(ctx context.Context) {
	...
	backoff.RunFWithTimeAfter(ctx, runStreamWithBackoff, s.backoff, s.timeAfter)
}
...
func (s *adsStreamImpl) sendMessageLocked(stream clients.Stream, names []string, url, version, nonce string, nackErr error) error {
	...
	if err := stream.Send(msg); err != nil {
		s.logger.Warningf("Sending ADS request for type %q, resources: %v, version: %q, nonce: %q failed: %v", url, wireNames, version, nonce, err)
		return err
	}
	...
	if s.logger.V(perRPCVerbosityLevel) {
		...
		s.logger.Infof("ADS request sent for type %q, resources: %v, version: %q, nonce: %q", url, wireNames, version, nonce)
	}

	return nil
}

func (s *adsStreamImpl) singleSubscribedTypeURL() string {
	s.mu.Lock()
	defer s.mu.Unlock()
	if len(s.resourceTypeState) == 1 {
...
func (s *adsStreamImpl) recv(stream clients.Stream) bool {
	...
	for {
		...
		if url == "" {
			if singleURL := s.singleSubscribedTypeURL(); singleURL != "" {
				url = singleURL
			}
		}
