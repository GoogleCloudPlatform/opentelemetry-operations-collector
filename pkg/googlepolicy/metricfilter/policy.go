...
// value." A field that is set to its default therefore reports exists=false
// while still returning its rendered value, so that string and numeric
// predicates can match it (see matcher.present).
