...
func TestMetricDescriptorFieldSelectors(t *testing.T) {
	...
	t.Run("empty descriptor strings report absent", func(t *testing.T) {
		...
		for _, field := range []policyv1alpha1.MetricDescriptorField{
			...
			assert.Equal(t, "", val, "field %v", field)
		}
	})
}
...
// it, so the "type" routing envelope is stripped by the registry before the
// driver decodes with DiscardUnknown: false.
		...
		},
		{
			...
			raw: map[string]any{
				...
				"matches": []any{
					map[string]any{
						"target": map[string]any{
							"descriptor_fields": "METRIC_DESCRIPTOR_FIELD_NAME", // typo
						},
						"exists": map[string]any{},
					},
				},
...
// first-class descriptor string fields: an empty (default) value reports
// exists=false per metric_filter_policy.proto's "non-default (non-empty /
// non-zero)" clause, yet is still handed to the value-comparing predicates, so
// `equals: {string_value: ""}` matches it.
func TestPresentButEmptyFields(t *testing.T) {
	...
	for _, field := range fields {
		t.Run(field.String(), func(t *testing.T) {
			...
			assert.Equal(t, "", val, "default value must still be present for value predicates")
			...
			assert.Equal(t, googlepolicy.EvalDrop, equalsPol.EvaluateMetric(ctx))

			regexPol := mustDropPolicy(t, "regex-empty-"+field.String(), target, &policyv1alpha1.MetricMatcher_Regex{Regex: "^$"}, false)
			assert.Equal(t, googlepolicy.EvalDrop, regexPol.EvaluateMetric(ctx))
		})
	}
}
