...
func (mtp *agentMetricsProcessor) ConsumeMetrics(ctx context.Context, metrics pdata.Metrics) error {
	...
	convertNonMonotonicSumsToGauges(metrics)

	if metrics, err = combineProcessMetrics(metrics); err != nil {
		errors = append(errors, err)
	}

	if err := splitReadWriteBytesMetrics(metrics); err != nil {
		errors = append(errors, err)
	}

	if err := mtp.appendUtilizationMetrics(metrics); err != nil {
		errors = append(errors, err)
	}
...
func newMetricWithName(metric pdata.Metric, name string) pdata.Metric {
	...
	case pdata.MetricDataTypeIntSum:
		new.IntSum().SetIsMonotonic(metric.IntSum().IsMonotonic())
		new.IntSum().SetAggregationTemporality(metric.IntSum().AggregationTemporality())
	case pdata.MetricDataTypeDoubleSum:
		new.DoubleSum().SetIsMonotonic(metric.DoubleSum().IsMonotonic())
		new.DoubleSum().SetAggregationTemporality(metric.DoubleSum().AggregationTemporality())
	}
