...
| `opentelemetry_version` | string | | **Required.** The OpenTelemetry Collector repository version that the distribution is based off of. Components that belong to a stable module set, i.e. those released at `v1.x.x`, are detected from the `versions.yaml` of the repository at this version, so there is no need to specify a stable version separately. |
