...
1. Change `opentelemetry_version` and `opentelemetry_contrib_version` (see [note](#updating-contrib-version)). Stable component versions no longer need to be looked up; they are read from the `versions.yaml` of the core and contrib repositories at the versions you specify.
