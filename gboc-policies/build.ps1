...
docker build --build-arg WINDOWS_VERSION=$env:_WINDOWS_VERSION -f Dockerfile.windows.build -t otelcol-google-policy-windows:latest .
if ($LASTEXITCODE -ne 0) {
...
docker save otelcol-google-policy-windows:latest -o $TarPath
if ($LASTEXITCODE -ne 0) {
