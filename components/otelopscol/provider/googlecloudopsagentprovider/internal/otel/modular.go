...
func configToYaml(config interface{}) ([]byte, error) {
	...
	// Configure mapstructure to convert commonconfig.Secret to string
	decodeHook := mapstructure.ComposeDecodeHookFunc(
		func(f reflect.Value, t reflect.Value) (interface{}, error) {
			if f.Type() == reflect.TypeOf(commonconfig.Secret("")) {
				return string(f.Interface().(commonconfig.Secret)), nil
			}
			return f.Interface(), nil
		},
	)

	configurator := &mapstructure.DecoderConfig{
		DecodeHook: decodeHook,
		Result:     &outMap,
	}

	decoder, err := mapstructure.NewDecoder(configurator)
	if err != nil {
		return nil, err
	}
