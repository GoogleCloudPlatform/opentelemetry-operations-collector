...
func assertEqual(t *testing.T, expected, actual pdata.Metrics) {
	...
	for i := 0; i < rmsAct.Len(); i++ {
		...
		for j := 0; j < ilmsAct.Len(); j++ {
			...
			// currently expect IL to always be empty
			assert.True(t, ilmAct.InstrumentationLibrary().Name() == "" && ilmAct.InstrumentationLibrary().Version() == "")
			assert.True(t, ilmExp.InstrumentationLibrary().Name() == "" && ilmExp.InstrumentationLibrary().Version() == "")
